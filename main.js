...
// Aluminium Box Animation configuration
const FOIL_TOTAL_FRAMES = 300;
const FOIL_START_FRAME = 1;
const FOIL_FRAME_PREFIX = './foil-frames/ezgif-frame-';
const FOIL_FRAME_SUFFIX = '.jpg';
...
// Aluminium Foil Box Element Selectors
const foilCanvas = document.getElementById('foil-canvas');
const foilCtx = foilCanvas ? foilCanvas.getContext('2d') : null;
const foilSection = document.getElementById('foil-showcase');
const foilText1 = document.getElementById('foil-text-1');
const foilText2 = document.getElementById('foil-text-2');
const foilText3 = document.getElementById('foil-text-3');
...
// Total images to load in preloader (includes Butter Paper, BOPP, Custom rolls, and Aluminium Box frames)
const GLOBAL_TOTAL_IMAGES = TOTAL_FRAMES + BOPP_TOTAL_FRAMES + 2 + FOIL_TOTAL_FRAMES;
...
// Scroll & Lerp variables for Aluminium Box animation
let foilProgress = 0;
let foilTargetFrame = 1;
let foilCurrentFrame = 1;

// 4. Image Preloading Phase (Parallel loading for all animation frames)
const preloadImages = () => {
  return new Promise((resolve) => {
    ...
    // 4. Preload Aluminium Foil Box Frames (001 to 300)
    for (let i = 0; i < FOIL_TOTAL_FRAMES; i++) {
      const img = new Image();
      const frameNum = FOIL_START_FRAME + i;
      const filename = `${FOIL_FRAME_PREFIX}${pad(frameNum)}${FOIL_FRAME_SUFFIX}`;
      img.onload = onImageLoaded;
      img.onerror = () => onImageError(filename);
      img.src = filename;
      foilImages[i] = img;
...
// 7. Aluminium Foil Box Canvas Drawing & Calibration
const drawFoilFrame = (frameIndex) => {
  if (!foilCanvas || !foilCtx) return;
  const index = Math.min(FOIL_TOTAL_FRAMES - 1, Math.max(0, Math.round(frameIndex - FOIL_START_FRAME)));
  let img = foilImages[index];
  
  // High-performance complete-frame rendering fallback to prevent flickering
  if (!img || !img.complete || img.naturalWidth === 0) {
    let found = false;
    for (let offset = 1; offset < FOIL_TOTAL_FRAMES; offset++) {
      const prevImg = foilImages[index - offset];
      if (prevImg && prevImg.complete && prevImg.naturalWidth > 0) {
        img = prevImg;
        found = true;
        break;
      }
      const nextImg = foilImages[index + offset];
      if (nextImg && nextImg.complete && nextImg.naturalWidth > 0) {
        img = nextImg;
        found = true;
        break;
  ...
  foilCtx.drawImage(img, drawX, drawY, drawWidth, drawHeight);
};

// 8. Scroll Position Tracking & UI Sync
...
const handleScroll = () => {
  ...
  // Calculate Aluminium Foil scroll scrubbing progress
  if (foilSection) {
    const rect = foilSection.getBoundingClientRect();
    const sectionHeight = foilSection.scrollHeight;
    const windowHeight = window.innerHeight;
    const scrollDepth = sectionHeight - windowHeight;
    
    if (rect.top <= 0) {
      // Section is in view / active scroll
      foilProgress = Math.min(1, Math.max(0, -rect.top / scrollDepth));
    } else {
      foilProgress = 0;
    }
    
    foilTargetFrame = FOIL_START_FRAME + Math.floor(foilProgress * (FOIL_TOTAL_FRAMES - 1));
  }
};

// 9. Continuous Animation Loop (lerp & requestAnimationFrame)
const tick = () => {
  if (isLoaded) {
    // 9.1 Main Butter Paper animation scrubbing
    ...
    // 9.2 BOPP Scroll Scrubbing & Smooth Lerp
    if (boppSection) {
    ...
    // 9.3 Aluminium Box Scroll Scrubbing & Smooth Lerp
    if (foilSection) {
      const foilFrameDiff = foilTargetFrame - foilCurrentFrame;
      if (Math.abs(foilFrameDiff) > 0.05) {
        foilCurrentFrame += foilFrameDiff * 0.07;
      } else {
        foilCurrentFrame = foilTargetFrame;
      }
      
      // Draw frame onto canvas
      drawFoilFrame(foilCurrentFrame);
      
      // Smooth lerped progress
      const smoothFoilPercent = (foilCurrentFrame - FOIL_START_FRAME) / (FOIL_TOTAL_FRAMES - 1);
      
      // Foil 3D Entry/Exit Perspective Transformations
      let foilEntryProgress = 1;
      if (smoothFoilPercent < 0.15) {
        foilEntryProgress = smoothFoilPercent / 0.15; // entry scale
      } else if (smoothFoilPercent > 0.85) {
        foilEntryProgress = (1 - smoothFoilPercent) / 0.15; // exit scale
      }
      
      const foilRotX = 12 * (1 - foilEntryProgress);
      const foilRotY = -8 * (1 - foilEntryProgress);
      const foilScale = 0.88 + 0.12 * foilEntryProgress;
      if (foilCanvas) {
        foilCanvas.style.transform = `translate(-50%, -50%) rotateX(${foilRotX}deg) rotateY(${foilRotY}deg) scale(${foilScale})`;
      }

      // Foil Sequential Heading Text Triggers
      if (smoothFoilPercent > 0.08 && smoothFoilPercent < 0.35) {
        foilText1.classList.add('active');
      } else {
        foilText1.classList.remove('active');
      }

      if (smoothFoilPercent >= 0.38 && smoothFoilPercent < 0.68) {
        foilText2.classList.add('active');
      } else {
        foilText2.classList.remove('active');
      }

      if (smoothFoilPercent >= 0.71 && smoothFoilPercent < 0.95) {
        foilText3.classList.add('active');
      } else {
        foilText3.classList.remove('active');
      }
    }
  }
...
const init = async () => {
  ...
  if (foilCanvas) {
    foilCanvas.width = window.innerWidth;
    foilCanvas.height = window.innerHeight;
  }
  
  // Render introductory default frames
  drawFrame(1);
  drawBoppFrame(BOPP_START_FRAME);
  drawFoilFrame(FOIL_START_FRAME);
